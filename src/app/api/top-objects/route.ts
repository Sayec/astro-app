...
function buildReason(
    moonIllum: number, maxAlt: number, currentAlt: number,
    isAboveHorizon: boolean, isInBestPeriod: boolean
): string {
    const parts: string[] = [];
    ...
    if (isAboveHorizon) parts.push(`obiekt teraz na wys. ${Math.round(currentAlt)}°`);
    else parts.push(`maks. wys. ${Math.round(maxAlt)}°`);
...
export async function GET(request: NextRequest) {
    try {
        ...
        try {
            const cached = await redis.get<{ objects: any[], moonIllumination: number }>(cacheKey);
        ...
        for (const obj of DSO_CATALOG) {
            ...
            if (vis.isAboveHorizon) score += Math.min(25, Math.max(0, vis.currentAltitude) * 0.5);
            score += (100 - moonIllum) * 0.25;
            if (isInBestPeriod) score += 20;

            let rating = score >= 75 ? 5 : score >= 60 ? 4 : score >= 45 ? 3 : score >= 30 ? 2 : 1;
            const reason = buildReason(moonIllum, vis.maxAltitude, vis.currentAltitude, vis.isAboveHorizon, isInBestPeriod);

            scored.push({
                id: obj.id, name: obj.name, type: obj.type, emoji: obj.emoji,
                rating, reason, maxAltitude: Math.round(vis.maxAltitude),
                currentAltitude: Math.round(vis.currentAltitude),
                isAboveHorizon: vis.isAboveHorizon, bestSeason: vis.bestSeason, score,
            });
        }
        ...
        const resultData = { objects: top, moonIllumination: moonIllum };
        
        try {
            await redis.set(cacheKey, resultData, { ex: 3600 }); // 1 godzina 
        } catch(e) {}

        return NextResponse.json<{ objects: any[], moonIllumination: number }>(resultData);
    } catch (error) {
        console.error('[TopObjects] Error:', error);
        return NextResponse.json({ error: 'Failed to compute top objects' }, { status: 500 });
    }
}
